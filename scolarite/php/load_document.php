...
if($data[1]!=''){
    ...
    $requete= "SELECT* FROM fichier WHERE id_expediteur='".$id."' AND $champs='".$val."' ORDER BY date DESC";
   ...
   // die($id);
    //Fonction qui charge les départements
    $requete= "SELECT* FROM fichier WHERE id_expediteur='".$id."' ORDER BY date DESC";
    foreach ($bdd->query($requete) as $row) {
        # code...
        $fileName=$row['libelle'];
