                ...
                $.ajax({
                    ...
                    success: function(reponse3) {
                        //alert(reponse3);
                         ...
                         var select_ens=document.getElementById('select_ens');
                         
                         //Lorsqu'on clique sur un département pour charger les enseignants
                         select_matiere.addEventListener('change',function(){
                             select_ens.innerHTML="<option value=''>-Sélectionner-</option>";       
                             var data3=this.value;
                             //alert(data2)
                             $.ajax({
                                 url: 'php/load_ens.php',
                                 type: 'POST',
                                 data: {data: JSON.stringify(data3)}, 
                                 success: function(reponse4) {
                                     //alert(reponse3);
                                       enseignants=JSON.parse(reponse4);
                                      ...
                                      //Boucle qui parcourt le tableau
                                      for (let i4 = 0; i4 < enseignants.length; i4++) {
                                          //On créer le option HTML
                                          options4[i4]=document.createElement('OPTION');
                                          var element4=enseignants[i4][1]+' '+enseignants[i4][2];
                                          //On met la valeur du département dans le option
                                          options4[i4].appendChild(document.createTextNode(element4));//Le tet qui sera visible dan le option
                                          //On met la valeur qui sera récupérée
                                          options4[i4].value=enseignants[i4][0];
                                          //On met le option créé dans le select
                                          select_ens.appendChild(options4[i4]);
